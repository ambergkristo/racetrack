...
(async () => {
  ...
  function publicHeaderCopy(snapshot = state.raceSnapshot) {
    if (
      route === "/next-race" &&
      snapshot.state !== "RUNNING" &&
      snapshot.lockedSession
    ) {
      return {
        subtitle: "Which finished drivers should proceed to the paddock?",
        body: "This board now shows the finished session roster while the next lineup is queued for the Safety Official.",
      };
    }
  ...
  function telemetryHeader() {
    ...
    const headerCopy = routeConfig.public ? publicHeaderCopy(snapshot) : null;
    const headerSubtitle = compactHomeHeader
      ? ""
      : routeConfig.staff
        ? routeConfig.body
        : headerCopy?.subtitle || routeConfig.subtitle;
    const headerCaption = compactHomeHeader || routeConfig.staff
      ? ""
      : `<p class="route-caption">${escapeHtml(headerCopy?.body || routeConfig.body)}</p>`;
