// Generated from client/src. Run `npm run sync:client` after editing source modules.
  ...
  function nextRacePanels() {
    ...
    const stagedSessionCandidate =
      state.raceSnapshot.currentSession || activeSession || null;
    const stagedNextSession =
      endSessionActive &&
      stagedSessionCandidate &&
      stagedSessionCandidate.id !== endedSession.id
        ? stagedSessionCandidate
        : null;
    ...
    const secondaryPanelMarkup = endSessionActive
      ? `
          <div class="session-board tone-safe next-race-secondary-board">
            ...
            ${rosterStrip(stagedNextSession, {
              emptyTitle: "No next session staged yet",
              emptyDetail: "Front desk has not staged the next lineup yet.",
              gridClass: "next-race-roster-grid next-race-secondary-roster-grid",
            })}
          </div>
        `
      : "";
    ...
    return [
      panel(
        ...
        `
          ...
          <div class="session-board-grid next-race-session-board-grid${finishedClass()}${endSessionActive ? " is-paddock-handoff" : ""}">
